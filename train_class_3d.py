...
def conduct_test(test1,test2,i):
    ...
    test_data = np.concatenate(test_data).reshape([-1, 1, _x_, _y_, 1])
...
mew1,var1 = tf.nn.moments(x, axes=[0,1,2],keep_dims=False)
...
mew1,var1  = tf.nn.moments(h_conv1, axes=[0,1,2],keep_dims=False)
...
mew1,var1 = tf.nn.moments(h_pool1,axes=[0,1,2],keep_dims=False)
...
mew1,var1 = tf.nn.moments(h_conv2,axes=[0,1,2],keep_dims=False)
...
mew1,var1 = tf.nn.moments(h_pool2,axes=[0,1,2],keep_dims=False)
...
mew1,var1 = tf.nn.moments(h_conv3,axes=[0,1,2],keep_dims=False)
...
#-------------------------------------------------------------------------------------------#*
                                                                                            #*
# 7x7x128       FULLY CONNECTED 1                                                           #*
# 3 classes -> FG - Kidney, Liv, and BG                                                     #*
W_fc1 = init_weight([7*7*128,500])                                                          #*
b_fc1 = init_bias([500])                                                                    #*
in_fc1 = tf.reshape(h_pool3,[-1,7*7*128])
# mew1,var1 = tf.nn.moments(fc1_input,axes=[0],keep_dims=True)
# in_fc1 = tf.nn.batch_normalization(fc1_input,mew1,var1,offset,scale,1e-5)
# Dropout1
keep_prob = tf.placeholder("float")
in_fc1 = tf.nn.dropout(in_fc1,keep_prob)
h_fc1 = tf.matmul(in_fc1,W_fc1)+b_fc1
...
b_fc2 = init_bias([3])
in_fc2 = tf.nn.dropout(h_fc1,keep_prob)
    ...
    for i in range(50):
      ...
      while True:
          ...
          if not ( c1 and c2 ):
              ...
              train_data = sess.run(tf.reshape(train_data,[-1,1,_x_,_y_,1]))
              train_targ = sess.run(tf.reshape(train_targ,[-1,1,3,1]))

              bundle = zip(train_data,train_targ)
              np.random.shuffle(bundle)
              train_data, train_targ = zip(*bundle)
              train_data = np.concatenate(train_data).reshape([-1,1,_x_,_y_,1])
              ...
              # DELETE THIS AFTER TESTING THE CODE
              """1."""
              for _ in range(20):
                conduct_test(test1,test2,0)
