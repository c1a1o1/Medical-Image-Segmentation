...
class Data:
    ...
    def refine_pos(self,nx,ny,nz,pos):
        ...
        pos = np.matrix(pos)
        a = np.where(pos[:,0]+x_rad <= nx)
        b = np.where(pos[:, 0]-x_rad >= 0)
        c = np.where(pos[:,1]+y_rad <= ny)
        d = np.where(pos[:,1]-y_rad >= 0)
    ...
    def process_data(self,id,p_shape):
        ...
        offsets = []
        imset = []
        n_pos_samples = int(np.ceil(self.n_samples/2))
        ...
        while True:
            ...
            pos = np.where(lbl == id)
            neg_pos = np.where(lbl == self.neg_id)
            # Iterate until the constraints are met
            ...
            while not rand_pos_found and count < len(pos[0]):
                ...
                count += 1
                if (s_pos[0] - x_rad) > 0 and (s_pos[0] + x_rad) <= nx and \
                    (s_pos[1] - y_rad) > 0 and (s_pos[0] + y_rad) <= ny:
                    rand_pos_found = True
                else:
                    s_pos = []
                    continue
            ...
            if len(pos[0]) != 0:
                ...
                y_min = s_pos[1]-y_rad
                y_max = s_pos[1]+y_rad
                sample_patch = lbl[x_min:x_max,y_min:y_max,s_pos[2]]
                ...
                pos[0] += x_min
                pos[1] += y_min
                positions = self.refine_pos(nx,ny,1,pos)
                ...
                # rather a voxel positions...

                for i in range(len(offsets)):
                    patches = self.extract_patches(img, offsets[i], p_shape)
                    ...
                    for os in positions:
                        ...
                        x_rad = self.p_shape[0]/2
                        y_rad = self.p_shape[1]/2
                        patch = lbl[os[0]-x_rad:os[0]+x_rad,os[1]-y_rad:os[1]+y_rad,:]
                        ...
                        temp = img[neg_pos[0]-x_rad:neg_pos[0]+x_rad,neg_pos[1]-y_rad:neg_pos[1]+y_rad,neg_pos[2]]
                        temp = np.reshape(temp,[1,p_shape[0],p_shape[1],-1])
                        imset.append(temp)
    ...
    def extract_patches(self,img,offsets,p_shape):
        ...
        for i in range(len(offsets)):
            ...
            # z_min = _os[2] - p_radius[2]
            # z_max = _os[2] + p_radius[2]
            temp_img = img[x_min:x_max, y_min:y_max, _os[2]].astype('float64')
            temp_img = np.reshape(temp_img,[x_max-x_min,y_max-y_min,1])
...
# _data1 = Data(3,dir,[150,150,371],numeric_targ=True,n_samples=8,training=True)
...
# plt.figure()
# plt.title("Liver - 3")
# plt.imshow(train_data1[0,:,:,0],cmap="bone")
# plt.figure()
# plt.title("Kidney - 8")
